...
const Home: NextPage = () => {
  ...
  const realProcessMatches = useCallback(async () => {
    ...
    if (currentMatches.length > 0) {
      ...
      // Refill the board immediately without waiting for transactions
      setTimeout(() => {
        refillBoard(newBoard);
        setGameStatus("Ready for next move!");
      }, 800); // Keep a slight delay for visual effect
  ...
  const handleCandyClick = async (x: number, y: number) => {
    ...
    if (!selectedCandy) {
      ...
      if (isAdjacent) {
        ...
        // Check for matches after swap, but don't block UI
        setTimeout(async () => {
        ...
        <div className="h-full shadow-xl card bg-base-100">
          <div className="card-body">
            ...
            <div className="justify-center mt-6 card-actions">
              <button className="btn btn-primary btn-lg" onClick={initializeBoard}>
                Reset Game
              </button>
            </div>
          </div>
